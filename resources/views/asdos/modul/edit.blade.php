...
@section('title', 'Edit Modul - FlowTask')

@section('content')
<div class="px-6 py-4 max-w-2xl mx-auto">
  <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Modul: {{ $modul->nama_modul }}</h2>
  ...
  <form action="{{ route('asdos.modul.update', [$kelas->id, $modul->id]) }}" method="POST">
    @csrf @method('PUT')
    ...
    <div class="mb-4">
      ...
      <textarea id="deskripsi_modul"
                ...
                rows="4"
                class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-indigo-400">{{ old('deskripsi_modul', $modul->deskripsi_modul) }}</textarea>
    </div>
    ...
    <div class="mb-4">
      ...
      <input type="date"
             id="deadline"
             name="deadline"
             value="{{ old('deadline', $modul->deadline->format('Y-m-d')) }}"
             class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-indigo-400"
             required>
    </div>
